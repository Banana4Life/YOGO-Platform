...
public class Menu extends BasicGameState {
    ...
    public void init(GameContainer container, StateBasedGame game) throws SlickException {
        ...
        plasmaPlatform = new Animation(new SpriteSheet("res/images/platform/Plasma.png", 3, 3), 100);
    ...
    public void addHighlightEntry(Vector position, OnHighlightSelectListener listener) {
        ...
        if (selectedEntry >= 0) {
            ...
            if (highlightActive) {
                ...
                for (int n = 0; n < width; n++) {
                    plasmaPlatform.getImage(currentFrame).draw(highlightEntries.get(selectedEntry).position.x + (1 + n * 3) * scale, highlightEntries.get(selectedEntry).position.y, scale);
            ...
            engine.getSubImage(0, 0).draw(highlightEntries.get(selectedEntry).position.x + offsetLeft.x, highlightEntries.get(selectedEntry).position.y + offsetLeft.y, scale);
            engine.getSubImage(1, 0).draw(highlightEntries.get(selectedEntry).position.x + offsetRight.x + 3 * width * scale - 2 * scale, highlightEntries.get(selectedEntry).position.y + offsetRight.y, scale);
    ...
    public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
        ...
        offsetCounter++;
        if (offsetCounter >= 10) {
            offsetLeft = new Vector(generator.nextInt(4) - 2, generator.nextInt(4) - 2);
            offsetRight = new Vector(generator.nextInt(4) - 2, generator.nextInt(4) - 2);
        }
        offsetCounter %= 10;
        if (waitForExec) {
            waitedFramesCount++;

...
public class MainMenu extends Menu {
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g)
		...
		for (int i = 0; i < rows; i++) {
			float x_offset = (bikeDriving) ? (i % 2 == 0) ? rand_factor * i
					: -rand_factor * i : 0;
			g.drawAnimation(wheelAnim, bikePosition.x + 100 * bikeScale
					+ x_offset, 707f * bikeScale + i * bikeDimensions.y + i
					* spl);
			if (i % 2 == 0) {
			...
			g.drawImage(moto, bikePosition.x + x_offset, i * bikeDimensions.y
					+ i * spl);
		}
	...
	public void update(GameContainer container, StateBasedGame game, int delta)
		...
		if (bikeDriving && !bikePaused) {
			...
			if (bikePosition.x - rand_factor * (rows - 1) > game.getContainer()
					.getWidth()) {
				resetBike();
			}
		} else {
